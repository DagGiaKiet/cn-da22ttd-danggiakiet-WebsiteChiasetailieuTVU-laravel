...
<div class="max-w-7xl mx-auto">
    ...
    <div class="admin-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors rounded-xl shadow p-5">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-3xl font-bold">{{ number_format($revenue, 0, ',', '.') }} đ</div>
        </div>
        <span class="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded">Hôm nay</span>
      </div>
  ...
  <div class="admin-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors rounded-xl shadow p-5">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-3xl font-bold">{{ number_format($ordersCount) }}</div>
        </div>
        <span class="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded">Tuần này</span>
      </div>
      <div class="mt-3 w-full bg-gray-100 h-2 rounded">
  ...
  <div class="admin-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors rounded-xl shadow p-5">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-3xl font-bold">{{ number_format($leads) }}</div>
        </div>
        <span class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">Tháng này</span>
      </div>
      <div class="mt-3 w-full bg-gray-100 h-2 rounded">
  ...
  <div class="admin-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors rounded-xl shadow p-5">
      <div class="flex items-center justify-between">
        <div>
          <div class="text-sm text-gray-500">Lead Conversion Rate</div>
          <div class="text-3xl font-bold">{{ $conversionRate }} %</div>
        </div>
        <span class="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded">Tổng</span>
      </div>
      <div class="mt-3 w-full bg-gray-100 h-2 rounded">
  ...
  <div class="admin-card lg:col-span-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors rounded-xl shadow p-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <button class="text-gray-400 hover:text-gray-600"><i data-feather="more-horizontal"></i></button>
      </div>
  ...
  <div class="admin-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors rounded-xl shadow p-6">
      ...
      <ul class="space-y-3">
                <li class="flex items-center justify-between"><span class="text-gray-600">Người dùng</span> <span class="font-semibold">{{ number_format((int) $users) }}</span></li>
                <li class="flex items-center justify-between"><span class="text-gray-600">Tài liệu</span> <span class="font-semibold">{{ number_format((int) $documents) }}</span></li>
                <li class="flex items-center justify-between"><span class="text-gray-600">Bài blog</span> <span class="font-semibold">{{ number_format((int) $blogs) }}</span></li>
                <li class="flex items-center justify-between"><span class="text-gray-600">Đơn hàng</span> <span class="font-semibold">{{ number_format((int) $ordersCount) }}</span></li>
      </ul>
    </div>
  </div>
      ...
      <div class="divide-y">
        @forelse($recentOrders as $o)
          <div class="py-3 flex items-center justify-between">
            <div>
              <div class="font-medium">{{ $o->document->ten_tai_lieu ?? 'Tài liệu' }}</div>
              <div class="text-sm text-gray-500">{{ $o->user->name ?? 'Người dùng' }} • {{ $o->created_at->diffForHumans() }}</div>
            </div>
            <div class="text-right">
              <div class="font-semibold">{{ number_format((int)($o->document->gia ?? 0), 0, ',', '.') }} đ</div>
        ...
        @empty
          <div class="py-6 text-center text-gray-500">Chưa có đơn hàng nào.</div>
        @endforelse
      </div>
    </div>
  <div class="admin-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors rounded-xl shadow p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold">Kênh xã hội</h3>
      </div>
      <div class="space-y-4">
        <div class="flex items-center justify-between"><div class="flex items-center gap-2"><i data-feather="facebook" class="text-blue-600"></i><span>Facebook</span></div><span class="font-semibold">35000</span></div>
        <div class="flex items-center justify-between"><div class="flex items-center gap-2"><i data-feather="twitter" class="text-sky-500"></i><span>Twitter</span></div><span class="font-semibold">2500</span></div>
        <div class="flex items-center justify-between"><div class="flex items-center gap-2"><i data-feather="youtube" class="text-red-500"></i><span>YouTube</span></div><span class="font-semibold">1.7M</span></div>
      </div>
    </div>
  </div>
</div>
