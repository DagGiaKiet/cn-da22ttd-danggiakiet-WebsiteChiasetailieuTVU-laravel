...
CREATE TABLE `tin_nhan` (
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `tin_nhan_nguoi_gui_id_foreign` (`nguoi_gui_id`),
  KEY `tin_nhan_nguoi_nhan_id_foreign` (`nguoi_nhan_id`),
  CONSTRAINT `tin_nhan_nguoi_gui_id_foreign` FOREIGN KEY (`nguoi_gui_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `tin_nhan_nguoi_nhan_id_foreign` FOREIGN KEY (`nguoi_nhan_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
INSERT INTO `tin_nhan` (`id`, `nguoi_gui_id`, `nguoi_nhan_id`, `noi_dung`, `da_doc`, `created_at`, `updated_at`) VALUES
(1, 2, 3, 'Xin chào, bạn còn tài liệu Cơ sở Dữ liệu không?', 1, NOW(), NOW()),
(2, 3, 2, 'Còn bạn nhé! Bạn có thể gặp mình ở thư viện trường vào chiều mai.', 0, NOW(), NOW());
